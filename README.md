...
- **Same-origin enforced.** The server serves the client and the socket from one origin and sets no CORS headers. On top of that it validates the `Origin` header on uploads and on every socket handshake (browsers can't omit or spoof it), so a hostile web page you happen to visit can't write to your history or read it over a WebSocket. Requests without an `Origin` — curl and other non-browser clients — are allowed. None of this is a substitute for auth.
